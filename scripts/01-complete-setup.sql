...
-- This script creates the exact schema as it exists in the current database
-- Includes all tables, constraints, and indexes
-- NO DATA SEEDING - Only creates the schema structure
...
-- Create accrued values table (depends on users and jobs)
...
-- Create maintenance occurrences table (depends on users)
...
-- Create maintenance templates table (depends on users)
...
-- Create push subscriptions table (depends on users)
...
-- Create supervisor technicians table (many-to-many relationship between supervisors and technicians)
...
CREATE INDEX IF NOT EXISTS idx_maintenance_templates_recurrence ON maintenance_templates(recurrence_type, recurrence_interval);
